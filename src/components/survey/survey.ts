/**
 * `<nx-survey>`: una encuesta que da gusto contestar. Una pregunta a la vez, con el teclado
...
import { answersToSend, cleanQuestions, estimateMinutes, interpolate, isAnswered, letterOf, rangeOf, validate, visibleQuestions } from "./logic";
...
export class NxSurvey extends Base {
  ...
  back(): void {
    clearTimeout(this.#auto);
    if (this.#screen !== "question" || this.#step === 0) return;
    this.#step--;
    this.#error = "";
    this.#dir = "back";
    this.#save();
  ...
  #paintProgress(): void {
    ...
    this.#nav!.hidden = !q;
    this.#bar!.hidden = this.#screen === "intro";
  ...
  #render(focus = false): void {
    ...
    if (this.#screen === "intro") screen = this.#intro();
    else if (this.#screen === "done") screen = this.#done();
    else if (this.#screen === "sending") screen = h("div", { class: "nx-survey__screen nx-survey__sending", role: "status" }, h("span", { class: "nx-survey__spin", "aria-hidden": "true" }), L.sending);
    else {
      const vis = this.#visible();
      if (this.#step >= vis.length) this.#step = Math.max(0, vis.length - 1);
      const q = vis[this.#step];
      screen = q ? this.#question(q, this.#step === vis.length - 1) : h("div");
    }
    ...
    const target = screen.querySelector<HTMLElement>("[data-autofocus]") ?? screen.querySelector<HTMLElement>("input:checked") ?? screen.querySelector<HTMLElement>("legend, h2");
  ...
  #result(q: SurveyQuestion, r: SurveyQuestionResult, a: SurveyAnswer, n: (x: number) => string): HTMLElement {
    const L = this.#labels;
    ...
    return h("article", { class: "nx-survey__result" }, h("h4", null, title), ...body);
  }
}
